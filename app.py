...
def admin():
    ...
    con=db(); students=con.execute('SELECT * FROM students ORDER BY name').fetchall(); pending=con.execute('''SELECT books.*,students.name,students.class_name FROM books JOIN students ON students.id=books.student_id WHERE books.status='pending' ORDER BY books.id DESC''').fetchall()
    ranking=con.execute('''SELECT s.id,s.name,s.class_name,s.leadership,COUNT(b.id) AS approved_books FROM students s LEFT JOIN books b ON b.student_id=s.id AND b.status='approved' GROUP BY s.id,s.name,s.class_name,s.leadership ORDER BY approved_books DESC,s.name COLLATE NOCASE ASC''').fetchall(); con.close()
    ranking_rows=[]
    for pos,r in enumerate(ranking,1):
        approved=r['approved_books']; coins=approved*10
        if r['leadership']: stage='الريادة'; icon='🏆'
        elif approved>=25: stage='التميز'; icon='⭐'
        elif approved>=20: stage='الإنجاز'; icon='🎯'
        elif approved>=15: stage='المعرفة'; icon='💡'
        elif approved>=10: stage='الاستكشاف'; icon='🔎'
        else: stage='الانطلاقة'; icon='📖'
        ranking_rows.append({'rank':pos,'name':r['name'],'class_name':r['class_name'],'approved_books':approved,'coins':coins,'stage':stage,'icon':icon})
    return render_template('admin.html',students=students,pending=pending,ranking=ranking_rows)
